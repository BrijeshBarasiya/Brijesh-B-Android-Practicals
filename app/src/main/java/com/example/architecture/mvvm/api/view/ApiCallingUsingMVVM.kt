...
class ApiCallingUsingMVVM : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.userInfo.observe(this) {
            it?.let {
                Toast.makeText(applicationContext, this.getString(R.string.api_called_successfully), Toast.LENGTH_LONG).show()
            } ?: kotlin.run {
                Toast.makeText(applicationContext, getString(R.string.api_call_failed), Toast.LENGTH_LONG).show()
            }
        }
    }
